...
import { Eye, EyeOff } from "lucide-react"; // Menghapus BookOpen karena tidak terpakai
  ...
  const handleGoogleLogin = async () => {
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          // Pastikan redirect URL ini sudah terdaftar di Google Console & Supabase
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          <Link to="/" className="flex justify-center items-center">
            <img src={logo} alt="Logo PKBM" className="h-12 w-auto object-contain" />
          </Link>
          <CardTitle className="font-heading text-xl">Masuk</CardTitle>
          <CardDescription className="font-body">Pilih metode masuk ke akun Anda</CardDescription>
        </CardHeader>
